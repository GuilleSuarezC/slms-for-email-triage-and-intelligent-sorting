"""
llm_email_classifier_holdout_v4.py
===================================
...
Evaluación: Holdout estratificado (train_test_split) — una única partición
train/test, en lugar de validación cruzada (StratifiedKFold).
"""
...
TEST_SIZE = 0.3         # Proporción del holdout (test) estratificado
RANDOM_STATE = 42
SHUFFLE = True
...
def build_few_shot_examples_text(
    ...
    """
    Construye el bloque de texto de ejemplos Few-shot desde la partición de entrenamiento.

    RESTRICCIONES:
    - Solo usa email_corpus y class_label de train_df.
    - No usa ninguna otra columna.
    - No hay data leakage: train_df proviene exclusivamente de la partición de
      entrenamiento del holdout (nunca del holdout de test).
...
- Los ejemplos proceden únicamente de la partición de entrenamiento del holdout (sin data leakage).
...
def _cache_key(
    ...
    raw = f"{strategy}|holdout|{id_part}|{corpus_hash}|{model_name}"
...
# Contenedor mutable para el modelo, accesible desde run_holdout
_MODEL_REF = [None]
...
def run_holdout(
    ...
    """
    Ejecuta evaluación holdout (partición única train/test) para la estrategia dada.

    strategy: "zero_shot" | "few_shot"
    train_df: partición de entrenamiento (usada SOLO para construir ejemplos few-shot).
    test_df:  partición de test/holdout (sobre la que se evalúa el LLM).
    Devuelve un DataFrame con todas las predicciones sobre test_df.

    RESTRICCIÓN: el LLM recibe únicamente email_corpus. No hay data leakage:
    los ejemplos few-shot provienen exclusivamente de train_df.
    """
    logger.info(
        f"[{strategy.upper()}] Iniciando evaluación holdout | "
    ...
    # Construir ejemplos Few-shot desde la partición de entrenamiento (sin data leakage)
    few_shot_text = ""
    ...
    # ── Checkpoint: fichero CSV parcial ────────────────────────────────────
    # Se escribe en tiempo real: si el proceso cae, el trabajo no se pierde.
    checkpoint_path = output_dir / f"_checkpoint_{strategy}_holdout.csv"
    checkpoint_rows = []   # buffer de la ejecución actual

    # Clasificar cada correo de la partición de test/holdout
    for i, (idx, row) in enumerate(test_df.iterrows()):
        ...
        if reuse_cache:
            ...
            if prediction is not None:
                logger.debug(f"Cache hit: idx {idx}")

        if prediction is None:
            if is_empty_corpus:
                logger.warning(
                    f"email_corpus vacío en idx {idx}. "
        ...
        if (i + 1) % 10 == 0:
            logger.info(
                f"[{strategy.upper()}] Progreso: {i + 1}/{len(test_df)}"
            )
    ...
    try:
        ...
        logger.info(
            f"[{strategy.upper()}] Holdout completado. "
            f"Checkpoint final: {checkpoint_path.name}"
        )
    except Exception as e:
        logger.warning(f"Error al guardar checkpoint final: {e}")

    results_df = pd.DataFrame(all_results)
    accuracy_holdout = results_df["is_correct"].mean() if len(results_df) else float("nan")
    logger.info(
        f"[{strategy.upper()}] Evaluación holdout completada. "
        f"Total predicciones: {len(results_df)} | Accuracy: {accuracy_holdout:.4f}"
    )
    return results_df
...
def compute_metrics(
    ...
    """Calcula métricas sobre la partición de test/holdout."""
    metrics = {
        "strategy": strategy,
    ...
    for avg in ["macro", "weighted"]:
        try:
            ...
            # Verificar que hay suficiente diversidad de clases en el holdout
            ...
            logger.warning(
                f"[{strategy}] ROC AUC ({avg}) no calculable: {e}"
            )
...
def compute_all_metrics(
    ...
    """
    Calcula las métricas globales sobre la partición de test/holdout.
    Devuelve: global_metrics_dict
    """
...
def plot_confusion_matrix(
    ...
    """Gráfico 2/3: Matriz de confusión sobre la partición de test/holdout."""
    ...
    ax.set_ylabel("Real")
    ax.set_title(f"Matriz de Confusión (holdout) — {strategy.replace('_', '-')}")
    ...
    _safe_save(fig, plots_dir / f"0{'2' if strategy == 'zero_shot' else '3'}_confusion_matrix_{strategy}.png", logger)
...
def plot_f1_by_class(
    ...
    """Gráfico 4: F1-score por clase (sobre holdout) para ambas estrategias."""
    ...
    _safe_save(fig, plots_dir / "04_f1_by_class.png", logger)
...
def plot_roc_curves(
    ...
    """
    Gráfico 5: Curvas ROC One-vs-Rest sobre holdout (top max_classes clases por soporte).
    """
    ...
    _safe_save(fig, plots_dir / f"05_roc_curves_{strategy}.png", logger)
...
def generate_all_plots(
    ...
    """Genera y guarda todos los gráficos (evaluados sobre la partición de holdout)."""
...
def export_results(
    ...
    base_cols = [
        "email_id", "strategy", "true_label", "predicted_label",
    ...
    # --- Resumen de métricas globales (holdout) ---
    ...
    cols_error = [c for c in ["email_id", "strategy", "true_label",
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Clasificador de correos energéticos con LLM local (LM Studio) + Evaluación Holdout Estratificada"
    )
    parser.add_argument(
        "--input", type=str, default="data/email_dataset.csv",
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("INICIO DE EJECUCIÓN — LLM Email Classifier Holdout")
    logger.info(f"Timestamp: {datetime.now().isoformat()}")
    logger.info(f"Modelo: {MODEL_NAME}")
    logger.info(f"Estrategia de entrada predictiva: {PREDICTIVE_INPUT_COLUMN} (SOLO esta columna)")
    logger.info(f"Test size (holdout): {args.test_size} | Random state: {args.random_state}")
    ...
    run_config = {
        ...
        "note": (
            ...
            "Evaluación mediante holdout estratificado (una única partición "
            "train/test), no validación cruzada."
        ),
    ...
    # Partición holdout estratificada única (train / test)
    ...
    logger.info(
        f"Partición holdout: Train={len(train_df)} ({100*(1-args.test_size):.0f}%) | "
        f"Test={len(test_df)} ({100*args.test_size:.0f}%)"
    )
    run_config["n_train"] = len(train_df)
    run_config["n_test"]  = len(test_df)
    ...
    # MÉTRICAS
    # =========================================================================
    logger.info("Calculando métricas sobre el holdout...")

    global_metrics_zero = compute_all_metrics(results_zero, "zero_shot", logger)
    global_metrics_few  = compute_all_metrics(results_few,  "few_shot",  logger)

    # Logging de métricas globales
    logger.info("--- MÉTRICAS HOLDOUT: ZERO-SHOT ---")
    for k, v in global_metrics_zero.items():
        logger.info(f"  {k}: {v}")

    logger.info("--- MÉTRICAS HOLDOUT: FEW-SHOT ---")
